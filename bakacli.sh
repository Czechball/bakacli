...
#bash sha512 by prushik
#https://github.com/prushik/sha512.bash
...
getlogin()
{
	...
	if [[ $login == "" ]]
		then
			...
			if [[ `xidel -s $loginUrl?gethx=$login -e results/res` =~ 02 ]]
			then
				printf "Uživatel $login neexistuje\n"
				getlogin
			else
	...
	printf "Zadejte svoje heslo: "
	read password
	passsalt="$restyp$resikod$ressalt$password"
	passhash=$(sha512 $passsalt)
}

makeToken()
{
	token="*login*$login*pwd*$passhash*sgn*ANDR$date"
	tokenhx=$(sha512 $token | xxd -r -p | base64 | tr / _ | tr + -)
}
...
getlogin
makeToken
curl "$loginUrl?hx=$tokenhx&pm=rozvrh"
shell